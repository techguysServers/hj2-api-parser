...
fn parse_into_jobs(document: &Document) -> Result<Vec<Job>, String> {
    ...
    let jobs = children
        ...
        .map(|job| {
            ...
            Job {
                id: dictionary.get("referencenumber").unwrap().to_string(),
                schedule: dictionary.get("type").unwrap().to_string(),
                ...
                application_destination: dictionary.get("url").unwrap().to_string(),
                company: Company {
